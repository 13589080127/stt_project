...
print "The input file is %d bytes long" % len(indata)    #把indata的值带入到%d？  文件的长度，空格也算一个字符
...
#用法二：返回字符串长度
#str = "runoob"
#len(str)   #返回长度为6   一版来说，一个字符一个长度

#用法三：返回元素个数
#l = [1,2,3,4,5]   []集合，1,2,3,4,5元素
#len(l)   #返回个数为5


print "Dose the output file exist? %r" % exists (to_file)  #to_file文件名字符串作为exists方法的参数，
#print拼接，后面跟字符串、init类型、变量、bool类型（exists方法返回）
